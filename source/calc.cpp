...
void header() {
 ...
 try {
   ...
   if (argc < 2) {
     ...
     strncpy(script, argv[0], i+1);
	 script[i+1] = 0;
	 fprintf(stderr, "\n Full path = %s \n", script);
	 strcat (script, fname);
     fprintf(stderr, " File name = %s \n\n", script );
   }
   else strcpy(script, argv[1]);

   TokenString *TS;
   TS = new TokenString(script, EXTRA_PARAM_STRING, "", argc, argv);
   ...
   for (int step = 0; step < vary.steps; step++) {
     vary.step();
     TS = new TokenString(script, EXTRA_PARAM_STRING, vary.loopVarString, argc, argv); 
